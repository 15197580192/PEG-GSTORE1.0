...
void update(std::vector<GstoreConnector *> &servers, std::string &dbname, std::string query) {
    // query on each site.
    pthread_t thread[servers.size()];
    QNV qnv[servers.size()];
    for (int j = 0; j < servers.size(); j++)
    {
        qnv[j].queryId = j;
        qnv[j].serverId = j;
        qnv[j].server = servers[j];
        qnv[j].dbname = dbname;
        qnv[j].format = "json";
        qnv[j].query = query;
        std::cout << query << " on server " << j << " ";
        if (pthread_create(&thread[j], NULL, queryThread, &qnv[j]) != 0)
        {
            throw runtime_error("creates thread error!");
        }
        else
